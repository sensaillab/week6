    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `TodoItem` (`id` INTEGER NOT NULL, `name` TEXT NOT NULL, `qty` TEXT NOT NULL, PRIMARY KEY (`id`))');
  ...
  Future<List<TodoItem>> findAllItems() async {
    return _queryAdapter.queryList('SELECT * FROM TodoItem',
        mapper: (Map<String, Object?> row) => TodoItem(
            row['id'] as int, row['name'] as String, row['qty'] as String));
  }
